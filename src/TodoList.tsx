...
export default class TodoList extends React.PureComponent<TodoListProps,TodoListState> {
    ...
    render () {
        ...
        let todosFilter = todos.filter(Filters[filter])
        ...
        return <section className="todoapp">
            ...
            <section className="main">
                { todos.length > 0 &&
                <input className="toggle-all" type="checkbox" checked={this.remainingCount === 0} onChange={this.toggle}/>}
            ...
            <footer className="footer">
                {this.remainingCount > 0 && <span className="todo-count"><strong>{this.remainingCount}</strong> tache{this.remainingCount > 1 && 's'} à faire</span>}
                <ul className="filters">
                    <li>
                        <a href="#/" className={cx({selected: filter === 'all'})} onClick={this.setFilter('all')}>Tout voir</a>
                    </li>
                    <li>
                        <a href="#/active" className={cx({selected: filter === 'active'})} onClick={this.setFilter('active')} >A faire</a>
                    </li>
                    <li>
                        <a href="#/completed" className={cx({selected: filter === 'completed'})} onClick={this.setFilter('completed')} >Fait</a>
                    </li>
                </ul>
                { completedCount > 0 && <button className="clear-completed" onClick={this.clearCompleted}>Effacer</button>}
            </footer>
        </section>
    }
